...
const logTrade = async (ticker, cost, sold) => {
  ...
  if (profit === null) {
    console.log(
      chalk.red("failed to log trade due to profit calculation error.")
    );
    return;
  }
...
const displayTrades = () => {
  ...
  trades.forEach(({ ticker, cost, sold, profit }, index) => {
    ...
    console.log(
      chalk.magenta(`ticker: $${ticker}`),
      chalk.red(`\ncost: ${cost} ${config.crypto}`),
      chalk.green(`\nsold: ${sold} ${config.crypto}`),
      chalk.magenta(`\n${config.crypto.slice(0, 3)} profit: `) +
        (profit >= 0 ? chalk.green : chalk.red)(
          `${(sold - cost).toFixed(2)} ${config.crypto}`
        ),
      chalk.green(`\n${config.currency} profit: `) +
        (profit >= 0 ? chalk.green : chalk.red)(`$${profit.toFixed(0)}`)
    );
  });
